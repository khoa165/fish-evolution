...
/**
 * Driver class that runs the application. Recreate the game of big fish eating smaller fish.
 * 
 * @author Harry Le.
 */
public class FishEvolution extends PApplet {
  ...
  private ArrayList<EatableFish> fishEaten; // Fish can be eaten by player's fish.
  private ArrayList<EatableFish> eatenBySmallFish; // Fish can be eaten by small fish.
  private ArrayList<EatableFish> eatenByMediumFish; // Fish can be eaten by medium fish.
  private ArrayList<EatableFish> eatenByBlueShark; // Fish can be eaten by shark.
  private ArrayList<EatableFish> eatenByBeast; // Fish can be eaten by "beasts".

  // Fish generator rate control.
  private static int smallFishControl; // Control the rate of creating small fish.
  private static int mediumFishControl; // Control the rate of creating medium fish.
  ...
  private ArrayList<Button> mainButtons; // Collection of main buttons (always visible).
  // ArrayList of __X__Clicked means collection of displayed button when X button is clicked.
  ...
  private static int gameOverTimeControl; // Control the time the image appears after losing.
  ...
  private void setupGeneral() {
    ...
    Fish.setProcessing(this); // Set up PApplet field to pass into other classes.
  ...
  private void setupImages() {
    ...
    this.backgroundImage = this.loadImage("images" + File.separator + "OceanBackground.jpg");
    this.coralImage = this.loadImage("images" + File.separator + "Coral.png");

    // Set up other images.
    // Image when user's fish eats other fish.
    this.evaporateIcon = this.loadImage("images" + File.separator + "Evaporate.png");
  ...
  private void setupUserFish() {
    this.mainFish = new UserFish("UserFish1", 500, 300, this.fishEaten, this.swallowIcon);
    this.allFish.add(mainFish);
    // Swallow action helps to display evaporate icon when user's fish eats other fish.
  ...
  /**
   * Initialize fish generator rate to control the number of fish per unit of time.
   */
  ...
  private void setupButtons() {
    ...
    this.nameRequestButton =
        new FillTextButton("Please enter your name here!", 595, 265, this, 190, 30);
    ...
    // Buttons that appear after clicking the name request button and finishing entering name.
    this.gameInstructionButton =
        new Button(Config.getReadInstruction(this.languageDisplayed), 595, 335, this, 230, 30);
    this.newGameButton =
        new Button(Config.getStartGame(this.languageDisplayed), 595, 375, this, 230, 30);
    // >>> Adding those buttons to the corresponding ArrayList.
    ...
    this.returnFromInstructionButton = new Button(
        Config.getReturnFromInstruction(this.languageDisplayed), 595, 520, this, 590, 40);
    // >>> Adding those buttons to the corresponding ArrayList.
    ...
    this.progressToGameButton =
        new Button(Config.getPositiveConfirm(this.languageDisplayed), 595, 275, this, 360, 50);
    this.declineToPlayButton =
        new Button(Config.getNegativeConfirm1(this.languageDisplayed), 595, 350, this, 360, 50);
    // >>> Adding those buttons to the corresponding ArrayList.
    ...
    this.restartButton =
        new Button(Config.getRestartInLanguage(this.languageDisplayed), 395, 325, this, 100, 50);
    this.resumeButton =
        new Button(Config.getResumeInLanguage(this.languageDisplayed), 595, 325, this, 100, 50);
    this.logoutButton =
        new Button(Config.getLogoutInLanguage(this.languageDisplayed), 795, 325, this, 100, 50);
    // >>> Adding those buttons to the corresponding ArrayList.
    ...
    this.difficultyLevelButton =
        new Button(Config.getDifficultyLevel(this.languageDisplayed), 395, 275, this);
    this.languageButton = new Button(Config.getLanguage(this.languageDisplayed), 395, 350, this);
    this.performanceButton =
        new Button(Config.getPerformance(this.languageDisplayed), 595, 275, this);
    this.rankingsButton = new Button(Config.getRankings(this.languageDisplayed), 595, 350, this);
    this.soundButton = new Button(Config.getSoundOn(this.languageDisplayed), 795, 275, this);
    this.gameHistoryButton = new Button(Config.getHistory(this.languageDisplayed), 795, 350, this);
    // >>> Adding those buttons to the corresponding ArrayList.
  ...
  private void setupGameRunning() {
    this.runApplication = false; // Initialized as false, true only when new profile is created.
    this.gameOver = false; // Initialized as false, true when user loses game.
    FishEvolution.gameOverTimeControl = 1; // Control the time the image appears after losing.
  }
  ...
  private void setupUserName() {
    ...
    this.alreadyFillName = false; // Initialized as false till user fills his/her name.
  }
  ...
  private void drawButtonBarAndOtherButtons() {
    ...
    this.rect(890, 0, 1190, 30); // Box containing power and protein information.
    this.fill(255, 0, 0); // // Set the fill color to red.
    this.text(Config.getPlayerInLanguage(this.languageDisplayed) + ": " + this.nameAppear + "   "
        + Config.getLevelInLanguage(this.languageDisplayed) + ": " + this.mainFish.getPower()
        + "   " + "XP: " + this.mainFish.getProtein() + " / "
        + this.mainFish.getEvolutionRequirement(), 1040, 15); // Print player's name, level, XP.
    for (int i = 0; i < this.mainButtons.size(); ++i) { // Loop to draw main buttons.
      this.mainButtons.get(i).draw();
    }
    for (Button button : this.displayedButtons) { // Loop to draw buttons that need displaying.
      button.draw();
    }
  ...
  private void createLivingOceanCreature() {
    for (int i = 0; i < this.allFishTypes.size(); ++i) { // Loop through all fish types to create.
      this.controlFishRate(this.allFishTypes.get(i));
    }
  }

  /**
   * Control the rate of creating corresponding fish according to typeFish given.
   * 
  ...
  private void controlFishRate(String typeFish) {
    if (typeFish.equals("SmallFish")) { // Small fish.
      if (FishEvolution.smallFishControl % 10 == 0) { // Create 1 every 10 update.
      ...
      if (FishEvolution.mediumFishControl % 80 == 0) { // Create 1 every 80 update
        createSpecificFish(typeFish);
      }
      ...
      if (FishEvolution.blueSharkControl % 150 == 0) { // Create 1 every 150 update.
        createSpecificFish(typeFish);
      }
  ...
  private void createSpecificFish(String typeFish) {
    if (typeFish.equals("SmallFish")) {
      EatableFish aSmallFish = // Create a small fish.
          new SmallFish("SmallFish", 0, this.rand.nextInt(500) + 50, this.eatenBySmallFish);
      ...
      MediumFish aMediumFish = // Create a medium fish.
          new MediumFish("MediumFish", -100, this.rand.nextInt(500) + 50, this.eatenByMediumFish);
      this.allFish.add(aMediumFish); // Add to the collection of all fish.
      this.fishEaten.add(aMediumFish); // User's fish can eat medium fish (with condition).
      ...
      BlueShark aBlueShark =
          new BlueShark("BlueShark", -199, this.rand.nextInt(300) + 150, this.eatenByBlueShark);
      this.allFish.add(aBlueShark); // Add to the collection of all fish.
      this.fishEaten.add(aBlueShark); // User's fish can eat blue shark (with condition).
      this.eatenByBeast.add(aBlueShark); // Beast can eat blue shark.
    } else {
      // TODO needs more beasts beside whale, also makes whale can eat other animals.
      this.allFish.add(new UnbeatableBeast("KillerWhale", 1100, this.rand.nextInt(500) + 50));
    }
  }
  ...
  private void updateFish() {
    if (gameOver) { // Check for game over.
      if (FishEvolution.gameOverTimeControl % 75 == 0) {
        ...
        // Add fish representation of game over (like dead fish) to trigger game over image.
    ...
    for (int i = 0; i < this.allFish.size(); ++i) { // Loop through all fish.
      Swallowing fishSwallowing = this.allFish.get(i).update(this.mainFish.getXIconCoordinate(),
          this.mainFish.getYIconCoordinate()); // Update fish.
      if (fishSwallowing != null) { // Only non-null if it's user's fish and some other conditions.
        if (this.allFish.get(i) instanceof UserFish) { // Check for safety.
          if (this.mainFish.checkIfAlive()) { // Check alive.
            ...
            this.swallowIcon = new Swallowing(this.evaporateIcon, true);
            this.swallowIcon.swallow(this.allFish); // Activate the evaporate icon.
  ...
  private void removeAllIcon() {
    for (int i = 0; i < this.allFish.size(); ++i) { // Loop through all fish.
      // Eliminate all real fish (EatableFish, UnbeatableBeast) which leaves only "fake" fish that
      // represents icon.
      if ((this.allFish.get(i) instanceof Fish) && !(this.allFish.get(i) instanceof EatableFish)
          && !(this.allFish.get(i) instanceof UnbeatableBeast)) {
        Fish fishRepresentIcon = (Fish) this.allFish.get(i); // Get fish instance.
  ...
  public void mousePressed() {
    ...
    // Helper method for button to confirm progress to play game or decline and log out.
  ...
  private void mainButtonMousePressed() {
    if (this.newProfileButton.isMouseOver()) { // User clicks new profile option.
      this.newProfileMousePressed(); // Call another helper method for new profile button.
      this.latestClickedButton = this.newProfileButton; // Change the latest clicked button.
    } else if (this.pauseGameButton.isMouseOver()) { // User clicks pause game option.
      this.pauseGameMousePressed(); // Call another helper method for pause game button.
      this.latestClickedButton = this.pauseGameButton; // Change the latest clicked button.
    } else if (this.settingsButton.isMouseOver()) { // User clicks settings option.
      this.settingsMousePressed(); // Call another helper method for settings button.
      this.latestClickedButton = this.settingsButton; // Change the latest clicked button.
    }
  }
  ...
  private void newProfileMousePressed() {
    this.displayedButtons = new ArrayList<Button>(); // Empty the displayed button collection.
    this.displayedButtons.addAll(this.newProfileClicked); // Add all sub-buttons.
    this.mainButtonClickedDisplay(); // Background box that contains text and buttons.
    this.text(Config.getNameRequest(this.languageDisplayed), 585, 225); // Request name from user.
  }
  ...
  private void pauseGameMousePressed() {
    this.displayedButtons = new ArrayList<Button>(); // Empty the displayed button collection.
    this.displayedButtons.addAll(this.pauseGameClicked); // Add all sub-buttons.
    this.mainButtonClickedDisplay(); // Background box that contains text and buttons.
    this.text(Config.getPauseWarning(this.languageDisplayed), 592, 250); // Options when pause game.
  }
  ...
  private void settingsMousePressed() {
    this.displayedButtons = new ArrayList<Button>(); // Empty the displayed button collection.
    this.displayedButtons.addAll(this.settingsClicked); // Add all sub-buttons.
    this.mainButtonClickedDisplay(); // Background box that contains text and buttons.
    this.text(Config.getPresentOption(this.languageDisplayed), 592, 220); // Settings options.
  }
  ...
  private void nameRequestMousePressed() {
    if (this.latestClickedButton != null // Check user clicked new profile button.
        && this.latestClickedButton.equals(this.newProfileButton)) {
      if (this.displayedButtons.get(0).isMouseOver()) { // this.nameRequestButton.isMouseOver().
        this.latestClickedButton = this.nameRequestButton; // Update latest clicked button.
        this.displayedButtons = new ArrayList<Button>(); // Empty the displayed buttons collection.
      }
    }
  }
  ...
  private void gameInstructionMousePressed() {
    // Check if user already filled username and the latest clicked button is nameRequestButton.
    if (this.alreadyFillName && this.latestClickedButton != null
        && this.latestClickedButton.equals(this.nameRequestButton)) {
      if (this.displayedButtons.get(0).isMouseOver()) { // this.gameInstructionButton.isMouseOver().
        this.gameInstruction(); // Call helper method to display game instruction.
        this.latestClickedButton = this.gameInstructionButton; // Update latest clicked button.
        this.displayedButtons = new ArrayList<Button>(); // Empty the displayed buttons collection.
      }
    }
  }
  ...
  private void startNewGameMousePressed() {
    ...
    if (this.alreadyFillName && this.latestClickedButton != null
        && this.latestClickedButton.equals(this.nameRequestButton)) {
      if (this.displayedButtons.get(1).isMouseOver()) { // this.newGameButton.isMouseOver().
        this.mainButtonClickedDisplay(); // Background box that contains text and buttons.
        this.text(Config.getConfirmNewGame(this.languageDisplayed), 590, 225); // Confirm new game.
        this.displayedButtons = new ArrayList<Button>(); // Empty the displayed buttons collection.
        this.displayedButtons.addAll(this.startGameClicked); // Add all sub-buttons.
        this.latestClickedButton = this.newGameButton; // Update latest clicked button.
      }
    }
  }
  ...
  private void returnFromInstructionMousePressed() {
    if (this.latestClickedButton != null
        && this.latestClickedButton.equals(this.gameInstructionButton)) { // Check latest clicked.
      if (this.returnFromInstructionButton.isMouseOver()) { // Check if user clicks return.
        // Update latestClickedButton to the state before clicking the game instruction, which is
        // nameRequestButton.
        this.latestClickedButton = this.nameRequestButton;
        this.drawContentInProfileOption(); // Re-draw the screen after filling name.
        this.displayedButtons = new ArrayList<Button>(); // Empty the displayed buttons collection.
        this.displayedButtons.addAll(this.nameRequestClicked); // Add all sub-buttons.
      }
    }
  }
  ...
  private void progressOrDeclineMousePressed() {
    ...
    if (this.latestClickedButton != null && this.latestClickedButton.equals(this.newGameButton)) {
      if (this.displayedButtons.get(0).isMouseOver()) { // this.progressToGameButton.isMouseOver().
        this.displayedButtons = new ArrayList<Button>(); // Empty the displayed buttons collection.
  ...
  public void keyPressed() {
    ...
    if (this.latestClickedButton != null
        && this.latestClickedButton.equals(this.nameRequestButton)) { // Progress if clicked.
      if (this.key != FishEvolution.ENTER) { // Check if not ENTER key.
        if (this.key == FishEvolution.BACKSPACE) { // If BACKSPACE key, delete last char.
  ...
  private void drawContentInProfileOption() {
    ...
    this.rect(0, 200, 1189, 400); // Gray background that contains text and buttons.
    ...
    this.text(this.userName.toUpperCase(), 595, 235); // Print username.
    this.text(this.userName.toUpperCase(), 595, 235); // Print username.
    this.text(this.userName.toUpperCase(), 595, 235); // Print username (3 times to make it bolder).
    this.nameAppear = this.userName.toUpperCase(); // Copy userName to nameAppear to display.
    this.textFont(this.boldFont, 14); // Change font size to 14.
    this.fill(255); // Set the fill color to white.
    this.text(Config.getConfirmName(this.languageDisplayed) + this.userName.toUpperCase(), 595,
        285); // Confirm name statement.
    this.fill(255, 255, 0); // Set the fill color to yellow.
    this.text(Config.getWarningModifyName(this.languageDisplayed), 595, 300); // Warning statement.
  }
  ...
  private void gameInstruction() {
    ...
    for (int i = 0; i < Config.getInstruction(this.languageDisplayed).length; ++i) {
      this.text(Config.getInstruction(this.languageDisplayed)[i], 330, 150 + 25 * i);
    }
    this.textAlign(FishEvolution.CENTER, FishEvolution.CENTER); // Change back to standard align.
    this.fill(255); // Set the fill color to white.
    this.rect(300, 500, 890, 540); // Return from game instruction.
    this.fill(255, 0, 0); // Set the fill color to red.
    this.text(Config.getReturnFromInstruction(this.languageDisplayed), 595, 520);
  }
}
